...
class Docker {
  ...
  _args() {
    const environments = _get(this.options, 'env', []).flatMap((env) => {
    ...
    const envFile = this.options.envFile ? ['--env-file', this.options.envFile] : [];
    const additionals = this.options.addArgs ? this.options.addArgs.split(' ') : [];
